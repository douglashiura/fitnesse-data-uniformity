...
public class SlimCommandRunningClient implements SlimClient {
	...
	public void start() throws IOException, SlimVersionMismatch {
		try {
			...
			e.printStackTrace();
			final String slimErrorMessage = "Error SLiM server startup failed. " + slimRunner.getCommandErrorMessage();
	...
	private void checkForVersionMismatch() throws SlimVersionMismatch {
		...
		if (serverVersionNumber == NO_SLIM_SERVER_CONNECTION_FLAG) {
			throw new SlimVersionMismatch(
					"Slim Protocol Version Error: Server did not respond with a valid version number.");
		} else if (serverVersionNumber < requiredSlimVersion) {
			throw new SlimVersionMismatch(String.format("Slim Protocol Version Error: Expected V%s but was V%s",
					requiredSlimVersion, serverVersionNumber));
		}
	}
	...
	public void connect() throws IOException {
		...
		LOG.finest("Trying to connect to host: " + hostName + " on port: " + port + " timeout setting: "
				+ connectionTimeout);
		while (client == null) {
			if (slimRunner != null && slimRunner.isDead()) {
				final String slimErrorMessage = "Error SLiM server died before a connection could be established. "
	...
	protected void validateConnection() throws IOException {
		long timeOut = Clock.currentTimeInMillis() + connectionTimeout * 1000;
		LOG.finest("Trying to get SlimHeader: " + " timeout setting: " + connectionTimeout);
		while (!isConnected()) {
			if (slimRunner != null && slimRunner.isDead()) {
		...
		try {
			...
			throw new SlimError(
					"Got invalid slim version from Client. Read the following: " + slimServerVersionMessage);
		}
