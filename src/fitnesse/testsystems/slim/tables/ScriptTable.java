...
public class ScriptTable extends SlimTable {
	...
	protected List<SlimAssertion> actionAndAssign(String symbolName, int row) {
		...
		if (!actionName.equals("")) {
			...
			assertions
					.add(makeAssertion(callAndAssign(symbolName, getTableType() + "Actor", actionName, (Object[]) args),
							new SymbolAssignmentExpectation(symbolName, 0, row, table, testContext)));
	...
	protected List<SlimAssertion> action(int row) throws TestExecutionException {
		...
		if (assertions.isEmpty()) {
			// Invoke fixture:
			int lastCol = table.getColumnCountInRow(row) - 1;
			return invokeAction(0, lastCol, row, new ScriptActionExpectation(0, row, table, testContext));
		}
		return assertions;
	}
	...
	protected List<SlimAssertion> show(int row) {
		int lastCol = table.getColumnCountInRow(row) - 1;
		return invokeAction(1, lastCol, row, new ShowActionExpectation(0, row, table, testContext));
	}

	protected List<SlimAssertion> ensure(int row) {
		int lastCol = table.getColumnCountInRow(row) - 1;
		return invokeAction(1, lastCol, row, new EnsureActionExpectation(0, row, table, testContext));
	}

	protected List<SlimAssertion> reject(int row) {
		int lastCol = table.getColumnCountInRow(row) - 1;
		return invokeAction(1, lastCol, row, new RejectActionExpectation(0, row, table, testContext));
	...
	protected List<SlimAssertion> checkAction(int row) {
		...
		return invokeAction(1, lastColInAction - 1, row, new ReturnedValueExpectation(lastColInAction, row, table));
	}

	protected List<SlimAssertion> checkNotAction(int row) {
		int lastColInAction = table.getColumnCountInRow(row) - 1;
		table.getCellContents(lastColInAction, row);
		return invokeAction(1, lastColInAction - 1, row, new RejectedValueExpectation(lastColInAction, row, table));
	}
	...
	protected List<SlimAssertion> invokeAction(int startingCol, int endingCol, int row, SlimExpectation expectation) {
		...
		assertions.add(makeAssertion(callFunction(getTableType() + "Actor", actionName, (Object[]) args), expectation));
		return assertions;
	}
	...
	protected String[] getArgumentsStartingAt(int startingCol, int endingCol, int row, List<SlimAssertion> assertions) {
		...
		while (extractor.hasMoreToExtract()) {
			assertions.add(makeAssertion(Instruction.NOOP_INSTRUCTION,
					new ArgumentExpectation(extractor.argumentColumn, row, table, testContext)));
	...
	public class ScriptActionExpectation extends RowExpectation {
		private ScriptActionExpectation(int col, int row, Table table, SlimTestContext testContext) {
	...
	public class EnsureActionExpectation extends RowExpectation {
		public EnsureActionExpectation(int col, int row, Table table, SlimTestContext testContext) {
			super(col, row, table, testContext);
		}

		@Override
		protected SlimTestResult createEvaluationMessage(String actual, String expected) {
			return (actual != null && actual.equals(BooleanConverter.TRUE)) ? SlimTestResult.pass()
					: SlimTestResult.fail();
		}
	}

	public class RejectActionExpectation extends RowExpectation {
		public RejectActionExpectation(int col, int row, Table table, SlimTestContext testContext) {
			super(col, row, table, testContext);
		}
	...
	public class ArgumentExpectation extends RowExpectation {

		private ArgumentExpectation(int col, int row, Table table, SlimTestContext testContext) {
			super(col, row, table, testContext);
		}

		@Override
		public TestResult evaluateExpectation(Object returnValue) {
			table.substitute(getCol(), getRow(), replaceSymbolsWithFullExpansion(getExpected(), table, testContext));
			return null;
		}
