...
public class QueryTable extends SlimTable {
	...
	public SlimTestResult matchMessage(String actual, String expected) {
		...
		if (actual.equals(replaceSymbols(expected)))
			return SlimTestResult.pass(replaceSymbolsWithFullExpansion(expected, table, testContext));
		Comparator c = new Comparator(actual, expected);
		return c.evaluate();
	}
	...
	public List<SlimAssertion> getAssertions() throws SyntaxError {
		...
		SlimAssertion qi = makeAssertion(callFunction(getTableName(), "query"), new QueryTableExpectation());
	...
	protected ExecutionResult markRows(QueryResults queryResults, Iterable<MatchedResult> potentialMatchesByScore) {
		...
		return !unmatchedTableRows.isEmpty() || !unmatchedResultRows.isEmpty() ? ExecutionResult.FAIL
				: ExecutionResult.PASS;
	}

	protected void markMatchedRows(QueryResults queryResults, Iterable<MatchedResult> potentialMatchesByScore,
			List<Integer> unmatchedTableRows, List<Integer> unmatchedResultRows) {
	...
	protected void removeOtherwiseMatchedResults(Iterable<MatchedResult> potentialMatchesByScore,
			MatchedResult bestResult) {
	...
	protected void replaceAllvariablesInRow(int tableRow) {
		...
		for (int col = 0; col < columns; col++) {
			...
			table.substitute(col, tableRow, replaceSymbolsWithFullExpansion(contents, table, testContext));
		}
	}
	...
	protected TestResult markField(int tableRow, int matchedRow, int col, QueryResults queryResults) {
		...
		if (fieldName.startsWith(COMMENT_COLUMN_MARKER))
			...
			if (symbolName != null) {
				...
				if (testResult == null)
					testResult = SlimTestResult.fail(actualValue,
							replaceSymbolsWithFullExpansion(expectedValue, table, testContext));
