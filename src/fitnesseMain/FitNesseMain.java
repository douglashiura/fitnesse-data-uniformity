...
public class FitNesseMain {
	...
	public Integer launchFitNesse(Arguments arguments) throws Exception {
		...
		contextConfigurator = contextConfigurator.updatedWith(
				ConfigurationParameter.loadProperties(new File(arguments.getConfigFile(contextConfigurator))));
	...
	public Integer launchFitNesse(ContextConfigurator contextConfigurator) throws Exception {
		...
		ClassLoader classLoader = PluginsClassLoaderFactory
				.getClassLoader(contextConfigurator.get(ConfigurationParameter.ROOT_PATH));
	...
	private ServerSocket createServerSocket(FitNesseContext context, ClassLoader classLoader) throws IOException {
		...
		return (useHTTPS
				? new SslServerSocketFactory(sslClientAuth,
						SslParameters.createSslParameters(sslParameterClassName, classLoader))
	...
	private void logStartupInfo(FitNesseContext context) {
		// This message is on standard output for backward compatibility with Jenkins
		// Fitnesse plugin.
